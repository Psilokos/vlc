                        ...
                        vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    if (newval.psz_string && *newval.psz_string)
    {
        ...
        if (!strcmp(psz_variable, "zoom"))
            var_SetFloat(vout, psz_variable, atof(newval.psz_string));
        else
            var_SetString(vout, psz_variable, newval.psz_string);
    ...
    else
    {
        ...
        vlc_value_t val;
        var_Get(vout, psz_variable, &val);
        char *val_text;
        var_Change(vout, psz_variable, VLC_VAR_GETTEXT, &val_text);
        if (!val_text)
            val_text = strdup(psz_variable);

        size_t choices_count;
        vlc_value_t *choices;
        char **choices_text;
        var_Change(vout, psz_variable, VLC_VAR_GETCHOICES,
                   &choices_count, &choices, &choices_text);
        ...
        if (!strcmp(psz_variable, "zoom"))
            ...
            {
                msg_rc("| %f - %s%s",
                       choices[i].f_float, choices_text[i],
                       val.f_float == choices[i].f_float ? " *" : "");
                free(choices_text[i]);
            }
        else
        {
            ...
            {
                char const *selected =
                    !strcmp(val.psz_string, choices[i].psz_string) ? " *" : "";
                msg_rc("| %s - %s%s",
                       choices[i].psz_string, choices_text[i], selected);
                free(choices_text[i]);
                free(choices[i].psz_string);
            }
            free(val.psz_string);
        }
        msg_rc("+----[ end of %s ]", val_text);
...
{ VLC_UNUSED(cmd); VLC_UNUSED(old); VLC_UNUSED(cur); VLC_UNUSED(dummy);
    return vlc_player_aout_NextDevice(
            vlc_playlist_GetPlayer(((intf_thread_t *)obj)->p_sys->playlist));
}
