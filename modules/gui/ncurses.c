...
static void
player_on_media_subitems_changed(vlc_player_t *player,
                                 input_item_t *media,
                                 input_item_node_t *subitems,
                                 void *userdata)
{
    VLC_UNUSED(player); VLC_UNUSED(media); VLC_UNUSED(subitems);

    intf_sys_t *sys = (intf_sys_t *)userdata;
    vlc_playlist_PlayAt(sys->playlist, sys->plidx_added);
}
...
static int Open(vlc_object_t *p_this)
{
    ...
    sys->playlist_listener =
        vlc_playlist_AddListener(playlist, &playlist_cbs, sys, false);
    vlc_playlist_Unlock(playlist);
    ...
    vlc_player_t *player = vlc_playlist_GetPlayer(playlist);
    vlc_player_Lock(player);
    sys->player_listener = vlc_player_AddListener(player, &player_cbs, sys);
    vlc_player_Unlock(player);
