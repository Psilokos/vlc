...
static struct
{
    ...
    unsigned            refcount;
} va_instance = { PTHREAD_MUTEX_INITIALIZER, NULL, NULL, 0, 0 };
...
{
    pthread_mutex_lock(&va_instance.lock);
    assert(va_instance.refcount > 0 && dpy == va_instance.dpy);

    va_instance.render_targets = render_targets;
    va_instance.num_render_targets = num_render_targets;
    pthread_mutex_unlock(&va_instance.lock);
}
...
vlc_vaapi_GetRenderTargets(VADisplay dpy, VASurfaceID **render_targets)
{
    ...
    *render_targets = va_instance.render_targets;
    unsigned num_render_targets = va_instance.num_render_targets;
...
vlc_vaapi_ReleaseInstance(VADisplay *dpy)
{
    ...
    if (--va_instance.refcount == 0)
    {
        ...
        if (va_instance.render_targets != NULL)
        {
            vaDestroySurfaces(va_instance.dpy, va_instance.render_targets,
                              va_instance.num_render_targets);
            va_instance.render_targets = NULL;
            va_instance.num_render_targets = 0;
        }
