...
static int Open(vlc_object_t *obj)
{
    ...
    if (is_upload)
    {
        ...
        filter_sys->va_surface_ids =
            vlc_vaapi_CreateSurfaces(obj, filter_sys->dpy, DEST_PICS_POOL_SZ,
                                     &filter->fmt_out.video,
                                     VA_RT_FORMAT_YUV420);
        if (!filter_sys->va_surface_ids)
        {
        ...
        filter_sys->dest_pics =
            vlc_vaapi_PoolNew(obj, filter_sys->va_surface_ids,
                              DEST_PICS_POOL_SZ, &filter->fmt_out.video);
        if (!filter_sys->dest_pics)
        {
            vaDestroySurfaces(filter_sys->dpy, filter_sys->va_surface_ids,
                              DEST_PICS_POOL_SZ);
            vlc_vaapi_ReleaseInstance(filter_sys->dpy);
            free(filter_sys);
            return VLC_EGENERIC;
    ...
    {
        /* Don't fetch the vaapi instance since it's not created yet at this
         * point */
...
Close(vlc_object_t *obj)
{
    ...
    if (filter_sys->image_fallback.image_id)
        vaDestroyImage(filter_sys->dpy, filter_sys->image_fallback.image_id);
    if (filter_sys->dest_pics)
        picture_pool_Release(filter_sys->dest_pics);
    if (filter_sys->dpy != NULL)
        vlc_vaapi_ReleaseInstance(filter_sys->dpy);
...
static int CreateFallbackImage(filter_t *filter, picture_t *src_pic)
{
    ...
    int ret;
    if (fmts[i].fourcc == VA_FOURCC_NV12
     && !vaCreateImage(va_dpy, &fmts[i], src_pic->format.i_width,
                       src_pic->format.i_height, &filter_sys->image_fallback))
        ret = VLC_SUCCESS;
    else
        ret = VLC_EGENERIC;
