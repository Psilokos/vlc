...
# define GET_VA_FILTER_DESC(f)  ((va_filter_desc_t *)((filter_t *)f)->p_sys)
...
{
    return filter->fmt_out.video.i_chroma == VLC_CODEC_VAAPI_OPAQUE &&
                      ...
                      pf_init_specific_int_data_t pf_InitSpecificInternalData)
{
    ...
    vlc_va_GetInstance(&base->va.dpy);
    assert(base->va.dpy);
    ...
    base->dest_pics =
        vlc_va_PoolAlloc(VLC_OBJECT(filter), base->va.dpy, DEST_PICS_POOL_SZ,
...
{
    filter_t *const             filter = (filter_t *)obj;
    va_filter_desc_t *const     va_filter_desc = GET_VA_FILTER_DESC(filter);

    if (vlc_va_IsEntrypointAvailable(obj, va_filter_desc->dpy,
                                     VAEntrypointVideoProc)
        || vlc_va_CreateConfig(obj, va_filter_desc->dpy, VAProfileNone,
                               VAEntrypointVideoProc, NULL, 0,
                               &va_filter_desc->conf)
        || vlc_va_CreateContext(obj, va_filter_desc->dpy, va_filter_desc->conf,
                                filter->fmt_out.video.i_width,
                                filter->fmt_out.video.i_height,
                                0, NULL, 0, &va_filter_desc->ctx))
        return VLC_EGENERIC;
...
{
    va_filter_desc_t *const     va_filter_desc = GET_VA_FILTER_DESC(obj);

    return vlc_va_IsVideoProcFilterAvailable(obj, va_filter_desc->dpy,
                                             va_filter_desc->ctx, filter_type);
}
                              ...
                              void *caps, unsigned int *p_num_caps)
{
    ...
    return vlc_va_QueryVideoProcFilterCaps(VLC_OBJECT(filter),
                                           va_filter_desc->dpy,
                                           va_filter_desc->ctx,
                         ...
                         pf_init_va_filter_params_t pf_InitVAFilterParams)
{
    ...
    ret_status = pf_InitVAFilterParams(filter, filter_params, &va_params,
                                       &sz_param, &num_params)
        || vlc_va_CreateBuffer(VLC_OBJECT(filter),
 ...
 pf_setup_specific_pipeline_caps_t pf_SetupSpecificPipelineCaps)
{
    ...
    if (vlc_va_QueryVideoProcPipelineCaps(VLC_OBJECT(filter), base->va.dpy,
 ...
 pf_release_specific_int_data_t pf_ReleaseSpecificInternalData)
{
    ...
    if (base->va.buf != VA_INVALID_ID)
        vlc_va_DestroyBuffer(obj, base->va.dpy, base->va.buf);
    if (base->va.ctx != VA_INVALID_ID)
        vlc_va_DestroyContext(obj, base->va.dpy, base->va.ctx);
    if (base->va.conf != VA_INVALID_ID)
        vlc_va_DestroyConfig(obj, base->va.dpy, base->va.conf);
    if (base->va.dpy)
        if (vlc_va_ReleaseInstance())
            msg_Err(obj, "no VA instance to release");
    free(filter->p_sys);
}
       ...
       pf_upd_specific_pipeline_params_t pf_UpdateSpecificPipelineParams)
{
    ...
    if (!Filter_GetDestPic(filter, src, &dest))
    {
        if (!pf_UpdateVAFilterParams
            ...
            if (!Filter_RenderPicture(filter, dest->p_sys->va_surface_id,
                                      src->p_sys->va_surface_id,
...
#ifndef NDEBUG
    dest_sys = (*p_dest)->p_sys;
#endif
    assert(dest_sys != NULL);
    assert(dest_sys->va_dpy == base->va.dpy);
                            ...
                            pf_upd_va_filter_params_t pf_UpdateVAFilterParams)
{
    ...
    return !vlc_va_MapBuffer(VLC_OBJECT(filter), va_filter_desc->dpy,
                             va_filter_desc->buf, &va_params)
        && !pf_UpdateVAFilterParams(filter, va_params)
        && !vlc_va_UnmapBuffer(VLC_OBJECT(filter),
 ...
 pf_upd_specific_pipeline_params_t pf_UpdateSpecificPipelineParams)
{
    ...
    return !vlc_va_BeginPicture(obj, base->va.dpy, base->va.ctx, dest_surface)
        && (!pf_PrepareRenderSurface || !pf_PrepareRenderSurface(filter))
        && !Filter_RenderSurface(filter, src_surface,
                                 pf_UpdateSpecificPipelineParams)
        && !vlc_va_EndPicture(obj, base->va.dpy, base->va.ctx)
        ? VLC_SUCCESS : VLC_EGENERIC;
}
 ...
 pf_upd_specific_pipeline_params_t pf_UpdateSpecificPipelineParams)
{
    ...
    if (vlc_va_CreateBuffer(obj, base->va.dpy, base->va.ctx,
                            VAProcPipelineParameterBufferType,
                            sizeof(*pipeline_params), 1, NULL, &pipeline_buf)
        || vlc_va_MapBuffer(obj, base->va.dpy, pipeline_buf,
    ...
    return !vlc_va_UnmapBuffer(obj, base->va.dpy, pipeline_buf)
        && !vlc_va_RenderPicture(obj, base->va.dpy, base->va.ctx,
                                 &pipeline_buf, 1)
        ? VLC_SUCCESS : VLC_EGENERIC;
}
